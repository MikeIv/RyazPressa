...
| _(не с бэка)_ `logoSrc` и др. | Логотип, favicon, touch-icon                       | Фронт строит из `slug`: `/sites/{slug}/logo.svg` и т.д.     | `shared/utils/siteThemeAssets.ts`                      | То же                                              |
...
- Бренд-ассеты (`logoSrc`, `faviconSrc`, `appleTouchIconSrc`, `logoAlt`) **не отдаются с бэка**. Фронт собирает пути по `slug` из статики билда (`/sites/<slug>/...`). См. `shared/utils/siteThemeAssets.ts`.
